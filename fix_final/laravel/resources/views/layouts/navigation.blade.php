<nav class="bg-white border-b border-gray-200 py-4 px-6 flex items-center justify-between relative">
    <div class="flex-none w-32">
        <a href="{{ route('dashboard') }}" class="text-2xl font-bold tracking-tighter text-black hover:text-gray-700">
            dribbble
        </a>
    </div>
    ...
    <div class="flex items-center flex-1 justify-center min-w-0 px-4">
        ...
        <div x-data="{ open: false }" class="relative flex-none mr-6">
            <button @click="open = !open" class="flex items-center font-bold text-sm border border-gray-200 rounded-xl px-4 py-2 hover:bg-gray-50 whitespace-nowrap focus:outline-none">
                <span>New & Noteworthy</span>
                <svg class="ml-2 w-4 h-4 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-show="open" 
                ...
                <a href="#" class="block px-5 py-2 text-sm text-gray-600 hover:bg-gray-50">Following</a>
    ...
    <div class="flex-none flex items-center justify-end space-x-5 min-w-[300px]">
        <div class="relative group hidden xl:block">
            ...
            <input type="text" placeholder="Search..." class="pl-10 pr-4 py-2 bg-gray-100 border-none rounded-full text-sm w-40 focus:ring-2 focus:ring-pink-100 focus:bg-white transition-all">
        </div>

        <button class="text-gray-500 hover:text-black transition-colors relative">
    ...
    <span class="absolute top-0 right-0 block h-2 w-2 rounded-full bg-pink-500 ring-2 ring-white"></span>
     ...
     src="https://ui-avatars.com/api/?name={{ Auth::check() ? urlencode(Auth::user()->full_name) : 'Guest' }}&color=7F9CF5&background=EBF4FF">
            </button>
...
<style>
    /* Hilangkan scrollbar tapi tetep bisa geser */
